...
class Settings(BaseSettings):
    """Runtime settings loaded from environment variables prefixed with WAI_AGENTS_."""
    ...
    model_config = {"env_prefix": "WAI_AGENTS_"}

    def validate_api_keys(self) -> None:
        """Raise RuntimeError if required API keys are missing."""
        if not self.anthropic_api_key:
            raise RuntimeError(
                "WAI_AGENTS_ANTHROPIC_API_KEY is not set. "
