...
from wai_agents_runtime.generated.waiagents.agent.v1 import agent_service_pb2 as pb2
...
BUILTIN_MCP_SERVERS: dict[str, dict[str, Any]] = {
    "memory": {
        ...
        "args": ["-m", "wai_agents_runtime.mcp.memory_server"],
        "env": {},
    },
    "web_search": {
        "name": "web_search",
        "transport": "stdio",
        "command": sys.executable,
        "args": ["-m", "wai_agents_runtime.mcp.web_search_server"],
        "env": {},
    },
    "code_exec": {
        "name": "code_exec",
        "transport": "stdio",
        "command": sys.executable,
        "args": ["-m", "wai_agents_runtime.mcp.code_exec_server"],
        "env": {},
    },
    "filesystem": {
        "name": "filesystem",
        "transport": "stdio",
        "command": sys.executable,
        "args": ["-m", "wai_agents_runtime.mcp.filesystem_server"],
        "env": {},
    },
}
...
def _resolve_builtin_mcp(
    ...
    for cfg in mcp_configs:
        ...
        if name in BUILTIN_MCP_SERVERS and not cfg.get("command"):
            ...
            builtin["env"] = {
                "WAI_AGENTS_AGENT_ID": agent_id,
                "WAI_AGENTS_USER_ID": user_id,
                "WAI_AGENTS_CONVERSATION_ID": conversation_id,
            }
