...
export const reflectionWorker = createWorker<ReflectionJobData>(
  QUEUE_NAME,
  async (job) => {
    ...
    const avgRating = ratingRows.length > 0
      ? (ratingRows as Array<Record<string, unknown>>).reduce((sum, r) => sum + (r['rating'] as number), 0) / ratingRows.length
      : null;
    ...
    if (suggestedRules.length > 0 || suggestedPreferences.length > 0) {
      ...
      const ownerId = agentOwnerRows.length > 0 ? (agentOwnerRows[0] as Record<string, unknown>)['creator_id'] as string : agentId;
