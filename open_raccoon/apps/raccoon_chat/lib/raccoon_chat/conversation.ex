defmodule RaccoonChat.Conversation do
  ...
  defp validate_type_constraints(changeset) do
    ...
    case type do
      :agent ->
        if is_nil(agent_id), do: add_error(changeset, :agent_id, "is required for agent conversations"), else: changeset
        |> then(fn cs -> if bridge_id, do: add_error(cs, :bridge_id, "must be nil for agent conversations"), else: cs end)

      :bridge ->
        if is_nil(bridge_id), do: add_error(changeset, :bridge_id, "is required for bridge conversations"), else: changeset
        |> then(fn cs -> if agent_id, do: add_error(cs, :agent_id, "must be nil for bridge conversations"), else: cs end)

      type when type in [:dm, :group] ->
        changeset
        |> then(fn cs -> if agent_id, do: add_error(cs, :agent_id, "must be nil for #{type} conversations"), else: cs end)
        |> then(fn cs -> if bridge_id, do: add_error(cs, :bridge_id, "must be nil for #{type} conversations"), else: cs end)
