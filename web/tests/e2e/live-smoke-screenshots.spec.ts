...
test('live smoke flow with screenshots: auth, chat, agents, feed, pages, marketplace, settings', async ({
  ...
  await expect(
    page.locator('[data-message-id]').filter({ hasText: messageText }).first(),
  ).toBeVisible();
  ...
  await page
    .locator('.ab-system-prompt')
    .fill('You are a concise assistant used for end-to-end smoke tests.');
  ...
  await expect(
    page
      ...
      .or(page.getByRole('button', { name: 'Liked' }))
      .first(),
  ).toBeVisible();
  ...
  const pagesUnavailable = page.getByText(
  ...
  if (pagesSupported) {
    ...
    await page
      .getByLabel('pages-module')
      .getByLabel('Slug')
      .fill(pageTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-'));
