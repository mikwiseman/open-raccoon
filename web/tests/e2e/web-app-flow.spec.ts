...
test("web app flow: auth, chat, feed, pages, marketplace, settings", async ({ page }) => {
  ...
  await expect(page.locator(".conversation-row.active").filter({ hasText: groupTitle })).toBeVisible();
  ...
  await expect(page.locator("[data-message-id]").filter({ hasText: editedMessageText }).first()).toBeVisible();

  const editedCard = page.locator("[data-message-id]").filter({ hasText: editedMessageText }).first();
  ...
  await expect
    .poll(async () => {
      ...
      const editedCount = await page.locator("[data-message-id]").filter({ hasText: editedMessageText }).count();
  ...
  const likeButton = page.locator("[data-feed-id]").first().getByRole("button", { name: /Like|Unlike/ });
  ...
  await expect
    .poll(async () => {
      const activeRowCount = await page.locator(".page-row.active").filter({ hasText: pageSlug }).count();
