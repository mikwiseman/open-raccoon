...
function ForkIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M5 8 Q5 10 8 11" />
      <path d="M11 8 Q11 10 8 11" />
    </svg>
  );
}
...
export function MarketplaceView({ api, currentUser, onOpenConversation }: MarketplaceViewProps) {
  ...
  const forkAgent = async () => {
    ...
    setForking(true);
    try {
      await api.forkFeedItem(detail.id);
      setInfo(`Forked "${detail.name}" to your agents`);
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
      setForking(false);
          ...
          {detail ? (
            <>
              ...
              <div className="mp-action-row">
                <button
                  type="button"
                  className="btn-primary mp-try-btn"
                ...
                <button
                  type="button"
                  className="mp-fork-btn"
                  onClick={() => void forkAgent()}
                  disabled={forking}
