...
export function PagesView({ api, currentUser }: PagesViewProps) {
  ...
  const createPage = async (event: FormEvent) => {
    ...
    try {
      ...
      setItems((previous) => [
        response.page,
        ...previous.filter((item) => item.id !== response.page.id),
      ]);
      ...
      {featureUnavailable && (
        <div className="pages-empty-detail" style={{ minHeight: 220 }}>
          ...
          <p style={{ color: 'var(--color-text-tertiary)' }}>
            The checked-in web client can still render this tab, but `GET /pages` and related routes
            are not exposed by the current TypeScript backend.
          </p>
        </div>
      )}
              ...
              <form className="pages-create-form" onSubmit={createPage}>
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setCreateVisibility(event.target.value as Page['visibility'])
                    }
                  >
              ...
              <form className="pages-detail-form" onSubmit={savePage}>
                ...
                <div className="pages-detail-meta">
                  <div className="pages-meta-row">
                    ...
                    <span className="pages-meta-value">
                      /{currentUser.username}/{selected.slug}
                    </span>
                  </div>
