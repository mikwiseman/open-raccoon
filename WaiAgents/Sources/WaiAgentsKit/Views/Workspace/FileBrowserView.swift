...
struct FileNodeRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Button {
                ...
                .padding(.leading, CGFloat(depth) * WaiAgentsSpacing.space4 + WaiAgentsSpacing.space2)
                .padding(.trailing, WaiAgentsSpacing.space3)
                .frame(height: 28)
    ...
    private func colorForNode(_ node: FileBrowserView.FileNode) -> Color {
        if node.isDirectory {
            return WaiAgentsColors.accentPrimary
        }
        let ext = (node.name as NSString).pathExtension.lowercased()
        switch ext {
