...
public struct AvatarView: View {
    ...
    public var agentAccentColor: Color = WaiAgentsColors.AgentAccent.default
    ...
    public init(
        ...
        isOnline: Bool = false,
        isAgent: Bool = false,
        agentAccentColor: Color = WaiAgentsColors.AgentAccent.default,
    ...
    public var body: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            if isOnline {
                Circle()
                    .fill(WaiAgentsColors.Semantic.success)
                    .frame(width: onlineIndicatorSize, height: onlineIndicatorSize)
                    .overlay {
                        Circle()
                            .strokeBorder(
                                colorScheme == .dark
                                    ? WaiAgentsColors.Dark.bgPrimary
                                    : WaiAgentsColors.Light.bgPrimary,
                                lineWidth: 2
                            )
                    }
                    .offset(x: 1, y: 1)
