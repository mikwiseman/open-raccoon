...
# Requires: helpers.sh, /tmp/wai_agents_test_tokens.env (ALICE/BOB/CHARLIE tokens)
...
make_request POST "/auth/register" \
  "{\"username\":\"\",\"email\":\"empty_user_${TIMESTAMP}@test.waiagents.dev\",\"password\":\"ValidP@ss123\"}"
...
make_request POST "/auth/register" \
  "{\"username\":\"'; DROP TABLE users; --\",\"email\":\"sqli_${TIMESTAMP}@test.waiagents.dev\",\"password\":\"ValidP@ss123\"}"
