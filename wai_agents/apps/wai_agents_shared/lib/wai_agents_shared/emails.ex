defmodule WaiAgentsShared.Emails do
  @moduledoc """
  Email templates for the WaiAgents platform.

  Uses Swoosh to compose emails that are delivered via the configured
  `WaiAgentsShared.Mailer` adapter (Resend in production, Local in dev).
  """

  import Swoosh.Email

  @from {"WaiAgents", "noreply@mail.waiwai.is"}
  ...
  def magic_link(email, token, base_url) do
    ...
    |> to(email)
    |> from(@from)
    |> subject("Your WaiAgents login link")
        ...
        <tr><td align="center">
          <table width="480" cellpadding="0" cellspacing="0" style="background:#1A1A1A;border-radius:12px;padding:40px;">
            <tr><td align="center" style="padding-bottom:24px;">
              <h1 style="color:#FFFFFF;font-size:24px;margin:12px 0 0;">WaiAgents</h1>
            </td></tr>
  ...
  def welcome(user) do
    ...
    |> to(user.email)
    |> from(@from)
    |> subject("Welcome to WaiAgents!")
    |> html_body(welcome_html(user))
    |> text_body(
      "Welcome to WaiAgents, #{user.display_name || user.username}! Your account is ready."
    )
  end
        ...
        <tr><td align="center">
          <table width="480" cellpadding="0" cellspacing="0" style="background:#1A1A1A;border-radius:12px;padding:40px;">
            ...
            <tr><td style="color:#A0A0A0;font-size:16px;line-height:24px;" align="center">
              Your WaiAgents account is ready. Start chatting with friends and AI agents.
            </td></tr>
          </table>
        </td></tr>
      </table>
