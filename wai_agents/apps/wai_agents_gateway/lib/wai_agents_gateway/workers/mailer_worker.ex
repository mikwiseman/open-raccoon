defmodule WaiAgentsGateway.Workers.MailerWorker do
  @moduledoc """
  ...
  def perform(%Oban.Job{args: %{"task" => "welcome", "user_id" => user_id}}) do
    case WaiAgentsAccounts.get_user(user_id) do
      nil ->
        {:discard, "User not found: #{user_id}"}

      user ->
        user
        |> WaiAgentsShared.Emails.welcome()
  ...
  defp deliver_email(email, task_type) do
    case WaiAgentsShared.Mailer.deliver(email) do
      {:ok, _} ->
        :ok
