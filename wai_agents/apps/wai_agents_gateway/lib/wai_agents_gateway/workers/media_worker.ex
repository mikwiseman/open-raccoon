defmodule WaiAgentsGateway.Workers.MediaWorker do
  @moduledoc """
  ...
  def perform(%Oban.Job{
    ...
    with {:ok, data} <- WaiAgentsShared.Media.R2.download(key) do
      ...
      case WaiAgentsShared.Media.R2.upload(thumb_key, data, content_type) do
        {:ok, _} -> :ok
  ...
  def perform(%Oban.Job{args: %{"task" => "transcode_video", "key" => key}}) do
    ...
    case WaiAgentsShared.Media.R2.download(key) do
      {:ok, _data} ->
        # Transcoding would happen here
        # mp4_key = String.replace(key, ~r/\.\w+$/, ".mp4")
        # {:ok, _} = WaiAgentsShared.Media.R2.upload(mp4_key, transcoded_data, "video/mp4")
        :ok
