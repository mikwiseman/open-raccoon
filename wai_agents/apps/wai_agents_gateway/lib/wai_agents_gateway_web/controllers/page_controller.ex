defmodule WaiAgentsGatewayWeb.PageController do
  ...
  def create(conn, params) do
    ...
    with {:ok, page} <- WaiAgentsPages.create_page(attrs) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    case WaiAgentsPages.get_page(id) do
      nil -> {:error, :not_found}
  ...
  def update(conn, %{"id" => id} = params) do
    ...
    case WaiAgentsPages.get_page(id) do
      ...
      %{creator_id: ^user_id} = page ->
        ...
        with {:ok, updated} <- WaiAgentsPages.update_page(page, update_params) do
  ...
  def deploy(conn, %{"id" => id} = params) do
    ...
    case WaiAgentsPages.get_page(id) do
      ...
      %{creator_id: ^user_id} = page ->
        ...
        with {:ok, updated_page} <- WaiAgentsPages.deploy_page(page, r2_path) do
          response = %{page: page_json(updated_page)}

          if idempotency_key = conn.assigns[:idempotency_key] do
            Idempotency.store(WaiAgentsShared.Repo, idempotency_key, user_id, 200, response)
          end
  ...
  def fork(conn, %{"id" => id} = params) do
    ...
    case WaiAgentsPages.get_page(id) do
      ...
      page ->
        ...
        with {:ok, forked} <- WaiAgentsPages.fork_page(page, user_id, new_slug) do
  ...
  def versions(conn, %{"id" => id}) do
    case WaiAgentsPages.get_page(id) do
      ...
      _page ->
        versions = WaiAgentsPages.list_versions(id)
