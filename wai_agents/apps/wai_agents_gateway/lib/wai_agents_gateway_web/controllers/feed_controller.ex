defmodule WaiAgentsGatewayWeb.FeedController do
  ...
  plug WaiAgentsGatewayWeb.Plugs.Idempotency when action in [:create, :fork]

  def index(conn, params) do
    {cursor, limit} = Pagination.parse_params(params)
    items = WaiAgentsFeed.list_feed(limit: limit + 1, cursor: cursor)
  ...
  def trending(conn, params) do
    {cursor, limit} = Pagination.parse_params(params)
    items = WaiAgentsFeed.list_trending(limit: limit + 1, cursor: cursor)
    {items, page_info} = Pagination.build_page_info(items, limit)
  ...
  def following(conn, params) do
    ...
    {cursor, limit} = Pagination.parse_params(params)
    items = WaiAgentsFeed.get_following(user_id, limit: limit + 1, cursor: cursor)
    {items, page_info} = Pagination.build_page_info(items, limit)
  ...
  def new_items(conn, params) do
    {cursor, limit} = Pagination.parse_params(params)
    items = WaiAgentsFeed.list_new(limit: limit + 1, cursor: cursor)
    {items, page_info} = Pagination.build_page_info(items, limit)
  ...
  def create(conn, params) do
    ...
    with {:ok, feed_item} <- WaiAgentsFeed.submit_item(attrs) do
      response = %{data: feed_item_json(feed_item)}

      if idempotency_key = conn.assigns[:idempotency_key] do
        Idempotency.store(WaiAgentsShared.Repo, idempotency_key, user_id, 201, response)
      end
  ...
  def like(conn, %{"id" => feed_item_id}) do
    ...
    with {:ok, feed_item_id} <- validate_uuid(feed_item_id) do
      case WaiAgentsFeed.get_feed_item(feed_item_id) do
        nil ->
          {:error, :not_found}

        _item ->
          case WaiAgentsFeed.like_item(feed_item_id, user_id) do
            {:ok, _like} ->
  ...
  def unlike(conn, %{"id" => feed_item_id}) do
    ...
    with {:ok, feed_item_id} <- validate_uuid(feed_item_id) do
      case WaiAgentsFeed.unlike_item(feed_item_id, user_id) do
  ...
  def fork(conn, %{"id" => feed_item_id}) do
    ...
    with {:ok, feed_item_id} <- validate_uuid(feed_item_id),
         {:ok, item} <- WaiAgentsFeed.fork_item(feed_item_id, user_id) do
