defmodule WaiAgentsGatewayWeb.UserController do
  ...
  def update(conn, params) do
    ...
    with {:ok, updated_user} <- WaiAgentsAccounts.update_user(user, update_params) do
      json(conn, %{user: user_json(updated_user)})
    end
  end

  def show(conn, %{"username" => username}) do
    case WaiAgentsAccounts.get_user_by_username(username) do
      nil -> {:error, :not_found}
