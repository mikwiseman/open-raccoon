defmodule WaiAgentsGatewayWeb.MarketplaceController do
  ...
  def index(conn, params) do
    ...
    agents = WaiAgentsAgents.list_public_agents(limit: limit + 1, cursor: cursor)
  ...
  def agent_profile(conn, %{"slug" => slug}) do
    case WaiAgentsAgents.get_agent_by_slug(slug) do
      nil ->
        {:error, :not_found}

      %{visibility: :public} = agent ->
        ratings = WaiAgentsAgents.get_agent_ratings(agent.id)
  ...
  def rate(conn, %{"id" => agent_id} = params) do
    ...
    with {:ok, agent_id} <- validate_uuid(agent_id) do
      ...
      with {:ok, rating} <- WaiAgentsAgents.rate_agent(attrs) do
        conn
        |> put_status(:created)
        |> json(%{
          rating: %{
  ...
  def search(conn, %{"q" => query} = params) do
    ...
    agents = WaiAgentsAgents.search_public_agents(query, limit: limit + 1, cursor: cursor)
    {items, page_info} = Pagination.build_page_info(agents, limit)
