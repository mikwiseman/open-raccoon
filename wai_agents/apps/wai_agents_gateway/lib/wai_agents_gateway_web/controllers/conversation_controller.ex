defmodule WaiAgentsGatewayWeb.ConversationController do
  ...
  def index(conn, params) do
    ...
    conversations =
      user_id
      |> WaiAgentsChat.list_user_conversations()
  ...
  def create(conn, params) do
    ...
    case check_dm_idempotency(attrs, user_id) do
      ...
      {:proceed, dm_member_id} ->
        ...
        with {:ok, conversation} <- WaiAgentsChat.create_conversation_with_members(attrs, members) do
          conn
          |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    ...
    with {:ok, id} <- validate_uuid(id) do
      case WaiAgentsChat.get_conversation(id) do
        nil ->
          {:error, :not_found}
      ...
      case WaiAgentsChat.get_conversation(id) do
        ...
        conversation ->
          ...
          with :ok <- ensure_moderator(id, user_id),
               {:ok, updated} <- WaiAgentsChat.update_conversation(conversation, update_params) do
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, id} <- validate_uuid(id) do
      case WaiAgentsChat.get_conversation(id) do
        ...
        conversation ->
          with :ok <- ensure_moderator(id, user_id),
               {:ok, _} <- WaiAgentsChat.delete_conversation_with_messages(conversation) do
    ...
    case normalize_conversation_type(attrs) do
      :dm ->
        ...
        cond do
          ...
          true ->
            case WaiAgentsChat.find_dm_between(user_id, other_user_id) do
