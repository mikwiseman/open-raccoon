defmodule WaiAgentsGatewayWeb.ConversationAccessControlTest do
  use WaiAgentsGatewayWeb.ConnCase, async: false
  ...
  setup %{conn: conn} do
    ...
    {:ok, conversation} =
      WaiAgentsChat.create_conversation(%{
        "type" => "group",
    ...
    {:ok, _message} =
      WaiAgentsChat.send_message(conversation.id, owner.id, %{
        "sender_type" => "human",
        "type" => "text",
    ...
    {:ok, other_message} =
      WaiAgentsChat.send_message(other_conversation.id, owner.id, %{
        "sender_type" => "human",
        "type" => "text",
  ...
  test "dm creation validates member_id and is idempotent", %{
    ...
    assert %{} = WaiAgentsChat.get_membership(created_id, owner.id)
    assert %{} = WaiAgentsChat.get_membership(created_id, member.id)
    ...
    assert length(WaiAgentsChat.list_members(created_id)) == 2
  end
