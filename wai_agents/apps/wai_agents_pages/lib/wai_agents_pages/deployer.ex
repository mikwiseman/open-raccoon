defmodule WaiAgentsPages.Deployer do
  @moduledoc """
  Page deployment to R2 storage.

  Handles uploading page bundles (HTML/CSS/JS) to R2,
  generating waiagents.page URLs, and rollback to previous versions.
  """

  alias WaiAgentsShared.Repo
  alias WaiAgentsShared.Media.R2
  alias WaiAgentsPages.{Page, PageVersion}
  import Ecto.Query
  ...
  @doc """
  Generate a unique waiagents.page URL for a page based on its slug.
  """
  @spec generate_url(Page.t()) :: String.t()
  def generate_url(%Page{slug: slug}) do
    base = Application.get_env(:wai_agents_pages, :pages_base_url, "https://waiagents.page")
    "#{base}/#{slug}"
  end
