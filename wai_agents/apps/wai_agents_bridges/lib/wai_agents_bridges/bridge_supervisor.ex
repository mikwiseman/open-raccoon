defmodule WaiAgentsBridges.BridgeSupervisor do
  @moduledoc """
  ...
  """
  @spec start_bridge(WaiAgentsBridges.BridgeConnection.t()) :: DynamicSupervisor.on_start_child()
  def start_bridge(bridge) do
    child_spec = %{
      id: bridge.id,
      start: {WaiAgentsBridges.BridgeWorker, :start_link, [bridge]},
      restart: :transient
    }
  ...
  defp find_child(bridge_id) do
    name = WaiAgentsBridges.BridgeWorker.via(bridge_id)
